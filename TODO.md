...
>
> GitHub issues tracked: [datvt243/nodejs-resume-api-ts](https://github.com/datvt243/nodejs-resume-api-ts/issues) — 8 closed, 5 open
...
| 1.5 | Password strength requirements | `config/regex.config.ts`, `plugins/joi/index.ts` | ✅ done | [#15](https://github.com/datvt243/nodejs-resume-api-ts/issues/15) min 12 chars, uppercase/lowercase/number/special |
...
| 1.9 | Forgot password / reset password | — | ❌ todo | [#15](https://github.com/datvt243/nodejs-resume-api-ts/issues/15) mentioned reset flow — not implemented |
...
| 5.4 | NoSQL injection prevention (QuerySafe) | `utils/querySafe.ts` | ✅ done | [#13](https://github.com/datvt243/nodejs-resume-api-ts/issues/13) Blocks `$` operators and `javascript:` patterns |
| 5.5 | Rate limiting (Redis + mem fallback) | `middlewares/rateLimit.middleware.ts` | ✅ done | [#14](https://github.com/datvt243/nodejs-resume-api-ts/issues/14) 100 req/15min general, 150 auth |
| 5.6 | CORS open to all origins | `config/cors.config.ts:8` | 🐛 bug/risk | `origin: '*'` — should restrict to known origins in production |
| 5.7 | HTTP security headers (Helmet) | — | ❌ todo | [#13](https://github.com/datvt243/nodejs-resume-api-ts/issues/13) mentioned helmet — not added |
...
| # | Feature | File(s) | Status | Notes |
|---|---|---|---|---|
| 7.1 | MongoDB connection (singleton) | `database/mongo.db.ts` | ✅ done | [#32](https://github.com/datvt243/nodejs-resume-api-ts/issues/32) updated |
| 7.2 | Redis client (singleton, optional) | `services/redis.ts` | ✅ done | Non-blocking init; graceful fallback |
| 7.3 | Winston logging (daily rotation) | `logger/` | ✅ done | [#17](https://github.com/datvt243/nodejs-resume-api-ts/issues/17) |
...
| 7.6 | Async/await patterns audit | various | ✅ done | [#20](https://github.com/datvt243/nodejs-resume-api-ts/issues/20) completed |
...
| 7.8 | MongoDB query optimization (indexes, pooling) | `database/mongo.db.ts`, `models/` | ❌ todo | [#23](https://github.com/datvt243/nodejs-resume-api-ts/issues/23) Add index on email, compound indexes, enable pooling |
| 7.9 | Docker / Docker Compose setup | — | ❌ todo | [#24](https://github.com/datvt243/nodejs-resume-api-ts/issues/24) Dockerfile + docker-compose (include Mongo) for dev + prod |
...
|---|---|---|---|---|
| 8.1 | auth.service — register, login, email check | `__tests__/auth/auth.service.test.ts` | ✅ done | [#19](https://github.com/datvt243/nodejs-resume-api-ts/issues/19) |
| 8.2 | auth.controller — all handlers | `__tests__/auth/auth.controller.test.ts` | ✅ done | [#19](https://github.com/datvt243/nodejs-resume-api-ts/issues/19) |
| 8.3 | Refresh token controller | `__tests__/auth/refreshToken.test.ts` | ✅ done | |
| 8.4 | verifyToken middleware | `__tests__/middlewares/verifyToken.test.ts` | ✅ done | [#18](https://github.com/datvt243/nodejs-resume-api-ts/issues/18) |
...
| # | Feature | Status | Notes |
|---|---|---|---|
| 9.1 | Swagger / OpenAPI spec | ❌ todo | [#22](https://github.com/datvt243/nodejs-resume-api-ts/issues/22) swagger-jsdoc + swagger-ui-express, document all endpoints with request/response schemas |
...
| 10.7 | DRY refactor — repeated code patterns | various | ❌ todo | [#21](https://github.com/datvt243/nodejs-resume-api-ts/issues/21) e.g. `getSelectFields → Object.keys`, consolidate validation helpers across CV section services |
